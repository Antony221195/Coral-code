function [u1,u2,u3]=Periodic_Rotlet_noslip_xyplane(xvec,yvec,zvec,Xs,Omega,wall,Lx,Ly)
...
%%.... Initialising the velocity profile for enface view

u1=zeros(length(yvec),length(xvec));
u2=zeros(length(yvec),length(xvec));
u3=zeros(length(yvec),length(xvec));

%.... As opposed to function file, over here, we use the traditional for
%looping to evaluate at each point without mesh generation. You can do this
%both ways

for jjj=1:length(yvec)
    y=yvec(jjj);
